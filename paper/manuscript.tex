...
Fig.~\ref{fig:koa_heatmap} visualizes the per-class F1 landscape as a heatmap, clearly showing how v7 and OWMixup variants elevate the darker (low-F1) regions in Grade~1. Fig.~\ref{fig:metrics} compares the validation metrics across six configurations.
...
\begin{figure}[htb]
\centering
\includegraphics[width=\linewidth]{fig2_barchart_KOA.png}
\caption{Validation metrics comparison on KOA. OWMixup variants outperform baseline on F1 and AUC.}
\label{fig:metrics}
\end{figure}
...
\textbf{Visual analysis.} Fig.~\ref{fig:gradcam} compares Grad-CAM attention maps between baseline and OWMixup, illustrating how ordinal-aware models focus on clinically relevant regions. Fig.~\ref{fig:tsne} shows t-SNE feature embeddings, revealing better class separation with OWMixup.

\begin{figure}[htb]
\centering
\placeholder[0.8]{5cm}
\caption{Grad-CAM comparison across methods. OWMixup highlights more focused anatomical regions.}
\label{fig:gradcam}
\end{figure}
